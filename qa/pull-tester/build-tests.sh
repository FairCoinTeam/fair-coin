...
cd /data/coins/gitian-builder/inputs/faircoin
make distdir
mkdir -p win32-build
...
cd linux-build
if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  ./configure --disable-silent-rules --disable-tests --disable-ccache --with-comparison-tool="$JAVA_COMPARISON_TOOL" --enable-comparison-tool-reorg-tests
else
  ./configure --disable-silent-rules --disable-tests --disable-ccache --with-comparison-tool="$JAVA_COMPARISON_TOOL"
fi
make -j$JOBS
...
if [ -d "$OUT_DIR" -a -w "$OUT_DIR" ]; then
  set +e
  # Windows:
  cp /data/coins/gitian-builder/inputs/faircoin/win32-build/src/bitcoind.exe $OUT_DIR/bitcoind.exe
  cp /data/coins/gitian-builder/inputs/faircoin/win32-build/src/qt/bitcoind-qt.exe $OUT_DIR/bitcoin-qt.exe
  # Linux:
  cp /data/coins/gitian-builder/inputs/faircoin/linux-build/src/bitcoind $OUT_DIR/bitcoind
  cp /data/coins/gitian-builder/inputs/faircoin/linux-build/src/qt/bitcoind-qt $OUT_DIR/bitcoin-qt
  set -e
fi

# Run unit tests and blockchain-tester on Linux:
cd /data/coins/gitian-builder/inputs/faircoin/linux-build
make check

# Run RPC integration test on Linux:
/data/coins/gitian-builder/inputs/faircoin/qa/rpc-tests/wallet.sh /data/coins/gitian-builder/inputs/faircoin/linux-build/src
/data/coins/gitian-builder/inputs/faircoin/qa/rpc-tests/listtransactions.py --srcdir /data/coins/gitian-builder/inputs/faircoin/linux-build/src
# Clean up cache/ directory that the python regression tests create
rm -rf cache

if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  # Run unit tests and blockchain-tester on Windows:
  cd /data/coins/gitian-builder/inputs/faircoin/win32-build
  make check
fi
...
# # Test code coverage
# cd /data/coins/gitian-builder/inputs/bitcoin
# make distdir
