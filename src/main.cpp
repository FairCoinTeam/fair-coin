...
bool CTransaction::CheckTransaction() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++)
    {
        ...
        if ((!txout.IsEmpty()) && txout.nValue < MIN_TXOUT_AMOUNT) {
            printf("minamount: %I64d      nValue: %I64d", MIN_TXOUT_AMOUNT, txout.nValue);
...
int64 GetProofOfWorkReward(unsigned int nHeight)
{
        ...
        if (nHeight == 1) 
			...
			nSubsidy = 500000 * COIN; // 50,000,000 coins
		else 
			nSubsidy = 0.1 * CENT; // negligeble POW Inflation
...
//vMerkleTree: ea6fed5e2
        // Genesis block
        const char* pszTimestamp = "2 Altcoin generators hit the marker, mayhem ensues";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0x236959d957876b91e6f2ade59f028b1742ff70e78712f6ba4a53efcaec7ac270"));
...
string GetWarnings(string strFor)
{
    ...
    // temp solution
	if (false/*Checkpoints::IsSyncCheckpointTooOld(60 * 60 * 24 * 90) && !fTestNet && !IsInitialBlockDownload()*/)
	{
		nPriority = 100;
